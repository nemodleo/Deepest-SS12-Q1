...
# from .augmentation.dataset_wrapper import DatasetWrapperFlower #DatasetWrapper
...
def build_loader(config):
    config.defrost()
    dataset_train, config.MODEL.NUM_CLASSES, flower_dataset = build_dataset(
        is_train=True, config=config)
    flower_dataset_train = flower_dataset[0]
    config.freeze()

    print(
        f"local rank {config.LOCAL_RANK} / global rank {dist.get_rank()} successfully build train dataset")
    dataset_val, _, flower_dataset_val = build_dataset(is_train=False, config=config)
    flower_dataset_val, flower_dataset_test = flower_dataset_val
    print(
    ...
    # data_loader_train_flower = torch.utils.data.DataLoader(
    #     flower_dataset_train, sampler=sampler_train_flower,
    #     batch_size=config.DATA.BATCH_SIZE,
    #     num_workers=config.DATA.NUM_WORKERS,
    #     pin_memory=config.DATA.PIN_MEMORY,
    #     # modified for TinyViT, we save logits of all samples
    #     drop_last=True#not config.DISTILL.SAVE_TEACHER_LOGITS,
    # )

    # data_loader_val_flower = torch.utils.data.DataLoader(
    #     flower_dataset_val, sampler=sampler_val_flower,
    #     batch_size=config.DATA.BATCH_SIZE,
    #     shuffle=False,
    #     num_workers=config.DATA.NUM_WORKERS,
    #     pin_memory=config.DATA.PIN_MEMORY,
    ...
    return dataset_train, dataset_val, flower_dataset_train, flower_dataset_val, flower_dataset_test, data_loader_train, data_loader_val,  mixup_fn


def build_dataset(is_train, config):

    transform, default_transform = build_transform(is_train, config)
    ...
    if is_train:
        train_dataset = Flowers102(root = '../dataset', split = "train", transform = transform, download=True)
        flower_dataset = (train_dataset)
    else:
        validation_dataset = Flowers102(root = '../dataset', split = "val", transform = default_transform)
        test_dataset = Flowers102(root = '../dataset', split = "test", transform = default_transform)
        flower_dataset = (
            validation_dataset,
            test_dataset
        )
    
    return dataset, nb_classes, flower_dataset
